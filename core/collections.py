...
def count_symbols(some_string):
  ...
  for key in some_string:
    d[key] = some_string.count(key)

  return d
# или с использованием dist comprehension
def count_symbols2(some_string):

  return {key:some_string.count(key) for key in some_string}